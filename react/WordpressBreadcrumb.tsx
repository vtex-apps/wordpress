      ...
      return (
        ...
        >
          {({
            ...
            if (data && data.wpCategory)
              return (
                <Container className={`${styles.breadcrumbContainer} pt2 pb8`}>
                  <Link
                    to={'/' + route}
                    className={`${styles.breadcrumbHomeLink}`}
                  >
                    Blog Home
                  </Link>
    ...
    if (this.props.params && this.props.params.terms) {
      return (
        <Container className={`${styles.breadcrumbContainer} pt2 pb8`}>
          ...
          <span className={`${styles.breadcrumbCurrentPage}`}>
            Search results for &quot;{this.props.params.terms}&quot;
          </span>
        </Container>
      )
    }
            ...
            if (data && data.wpPost)
              return (
                <Container className={`${styles.breadcrumbContainer} pt2 pb8`}>
                  ...
                  <Link
                    to={
                      '/' + route + '/category/' + data.wpPost.categories[0].id
                    }
...
export default compose(withSettingsNoSSR)(WordpressBreadcrumb)
